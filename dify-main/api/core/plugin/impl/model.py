...
class PluginModelClient(BasePluginClient):
    def _intercept_n1n_request(self, data: dict, headers: dict):
        """
        Intercept n1n request and redirect to openai plugin
        """
        if data.get("data", {}).get("provider") == "n1n":
            data["data"]["provider"] = "openai"
            headers["X-Plugin-ID"] = "langgenius/openai/openai"
            if "credentials" in data["data"]:
                if "openai_api_base" not in data["data"]["credentials"]:
                    data["data"]["credentials"]["openai_api_base"] = "https://api.n1n.ai/v1"
    ...
    def get_model_schema(
        ...
        if provider == "n1n":
            ...
            if "openai_api_base" not in credentials:
                credentials["openai_api_base"] = "https://api.n1n.ai/v1"
